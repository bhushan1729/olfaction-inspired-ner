"""
Training script for OntoNotes5 experiments.
Uses the same training logic as train.py but with OntoNotes5 data.
"""
...
from src.data.dataset_ontonotes import prepare_ontonotes_data, load_glove_embeddings_ontonotes
...
def train_epoch_ontonotes(model, train_loader, optimizer, device, config):
    """Train for one epoch."""
    model.train()
    total_loss = 0
    total_task_loss = 0
...
def train_ontonotes(config, experiment_name, save_dir):
    """Main training function for OntoNotes5."""
    ...
    # Prepare data
    print("\n" + "="*50)
    print("Loading OntoNotes5 data...")
    print("="*50)
    train_loader, valid_loader, test_loader, vocab_info = prepare_ontonotes_data(
        cache_dir=config.get('cache_dir', './data/ontonotes'),
    ...
    if config.get('use_pretrained_embeddings', True):
        ...
        if os.path.exists(glove_path):
            ...
            pretrained_embeddings = load_glove_embeddings_ontonotes(
                glove_path, 
                vocab_info['word2idx'], 
    ...
    # Tensorboard
    writer = SummaryWriter(os.path.join(config.get('tensorboard_dir', './runs/ontonotes'), experiment_name))
    ...
    for epoch in range(config.get('max_epochs', 50)):
        ...
        # Train
        train_metrics = train_epoch_ontonotes(model, train_loader, optimizer, device, config)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train NER model on OntoNotes5')
    parser.add_argument('--config', type=str, default='config/ontonotes_experiments.yaml',
                        help='Path to config file')
    parser.add_argument('--experiment', type=str, required=True,
                        help='Experiment name (e.g., baseline_ontonotes, olfactory_gelu_ontonotes)')
    parser.add_argument('--save_dir', type=str, default=None,
                        help='Directory to save results (default: results/ontonotes/<experiment>)')
    ...
    config = all_config[args.experiment]
    config.update(all_config.get('data', {}))
    config.update(all_config.get('training', {}))
    
    # Set save directory
    save_dir = args.save_dir if args.save_dir else os.path.join('results/ontonotes', args.experiment)
    
    # Train
    train_ontonotes(config, args.experiment, save_dir)
